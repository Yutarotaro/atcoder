...
int main() {
  ...
  vector<int> already_in(26);

  stack<int> right;

  vector<vector<int>> alphabets(1);
  int alphabets_idx = 0;

  rep(i, N) {
    if (s[i] == ')') {
      for(int j : alphabets[alphabets_idx]) { already_in[j] = 0; }
      alphabets[alphabets_idx].resize(0);
      --alphabets_idx;
    } else if (s[i] == '(') {
      // right.push(i);
      ++alphabets_idx;
      ...
      alphabets[alphabets_idx].push_back(idx);
    }

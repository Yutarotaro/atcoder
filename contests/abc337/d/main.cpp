...
int main() {
    ...
    vector<vector<pll>> a(H, vector<pll>(W));
    rep(i, H) { a[i][0] = {a[i][0] == 'o' ? 1 : 0, a[i][0] == '.' ? 1 : 0}; }

    rep(i, H) {
        for (int j = 1; j < W; ++j) {
            a[i][j] = {a[i][j - 1].first + a[i][j] == 'o' ? 1 : 0, a[i][j - 1].second + a[i][j] == '.' ? 1 : 0};
        }
    }

    rep(i, H) {
        rep(j, W - K + 1) {
            // judge from j to j + K - 1
            auto [p1, p2] = a[i][j];
            auto [p3, p4] = a[i][j + K - 1];

...
mint func(int n, int i) {
    if (seen[pll{n, i}]) {
        return memo[pll{n, i}];
    }

    if (n == 1) {
        return memo[pll{n, i}] = 1;
    }

    mint ret = 0;

    if (i == 1) {
        ret = func(n, n) * mint(2).inv();
    } else {
        ret = func(n - 1, i - 1) * mint(2).inv() + func(n, i - 1) * mint(2).inv();
    }

    seen[pll{n, i}] = 1;
    return memo[pll{n, i}] = ret;
}

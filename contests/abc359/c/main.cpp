...
int main() {
    ...
    if ((s.first + s.second) % 2) {
        s2 = {s.first - 1, s.second};
    } else {
        s2 = {s.first + 1, s.second};
    }
    if ((t.first + t.second) % 2) {
        t2 = {t.first - 1, t.second};
    } else {
        t2 = {t.first + 1, t.second};
    }

    ll y = abs(s.second - t.second);

    ll x;

    if (t.second % 2) {
        auto calc = [](ll a, ll b) {
            return abs(a + a % 2 - (b + b % 2)) / 2;
        };

        x = min({
            calc(s.first, t.first),
            calc(s2.first, t.first),
            calc(s.first, t2.first),
            calc(s2.first, t2.first),
        });

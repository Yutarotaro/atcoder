...
int main() {
    ...
    ll sum = 0;
    rep(i, N) { sum += players[i].second; }

    int winner_idx = sum % N;

    vector<pair<string, int>> players_idx(N);
    rep(i, N) { players_idx[i] = {players[i].first, i}; }

    sort(ALL(players_idx));

    cout << players[players_idx[winner_idx].second].first << endl;
}
